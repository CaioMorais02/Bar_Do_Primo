	...
	blt $t0, 4, pobre
		...
		while:
			blt $t0, 4, fim
				...
				if:
					#se nao for Budweiser, passa para a próxima
					bne $t1, 1, elseIf1
						blt $t0, 4, else
						sub $t2, $t0, 4
						move $t0, $t2
				...
				elseIf1:
					#se não for Amstel Lager Pilsen, passa para a próxima
					bne $t1, 2, elseIf2
						blt $t0, 9, else
						sub $t2, $t0, 9
						move $t0, $t2
				...
				elseIf2:
					#se não for Stela, passa para a próxima
					bne $t1, 3, elseIf3
						blt $t0, 10, else
						sub $t2, $t0, 10
						move $t0, $t2
				...
				elseIf3:
					#se não for Corona, passa para a próxima
					bne $t1, 4, elseIf4
						blt $t0, 11, else
						sub $t2, $t0, 11
						move $t0, $t2
				...
				elseIf4:
					#se n�o for Heineken, volta para o while
					bne $t1, 5, elseIf5
						blt $t0, 15, else
						sub $t2, $t0, 15
						move $t0, $t2
				...
				elseIf5:
					#se n�o for Pergola, volta para o while
					bne $t1, 6, elseIf6
						blt $t0, 16, else
						sub $t2, $t0, 16
						move $t0, $t2
				...
				elseIf6:
					#se n�o for Erdinger, volta para o while
					bne $t1, 7, elseIf7
						blt $t0, 23, else
						sub $t2, $t0, 23
						move $t0, $t2
				...
				elseIf8:
					#se n�o for Vodka Absolute, volta para o while
					bne $t1, 9, elseIf9
						blt $t0, 89, else
						sub $t2, $t0, 89
						move $t0, $t2
				...
				elseIf9:
					#se não for Red Label, volta para o while
					bne $t1, 10, elseIf10
						blt $t0, 99, else
						sub $t2, $t0, 99
						move $t0, $t2
				...
				elseIf10:
					#se não for Jack Daniels, volta para o while
					bne $t1, 11, elseIf11
						blt $t0, 130, else
						sub $t2, $t0, 130
						move $t0, $t2
				...
				elseIf11:
					#se não for Royal Salute, volta para o while
					bne $t1, 12, else
						blt $t0, 1289, else
						sub $t2, $t0, 1289
						move $t0, $t2
